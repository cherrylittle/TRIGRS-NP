subroutine rnoff_p(grd,sumex,imx1,celsiz,param,parami,nodat,nodata,mnd,sctr,ncol,nrow,header,test1,u,unum)
  ...
  integer:: i,k,kk,j,l,imx1,roflg,mnd !,m "m" removed 1 Feb 2013, RLB 
  integer:: grd,unum
  ...
  real (double):: time_st1,time_ed1,time_st2,time_ed2
  ...
  if(ans) then
     ...
     time_st2=MPI_WTIME()
     !do j=1,nper
    ! if(myrank.lt.nper) then
     !   j=myrank+1
        !write(*,*)'j=',j
		if(myrank.lt.nper) then
		do j=myrank+1,nper,isize
		!if(myrank.eq.0) j=1
		write(*,*) 'myrank=',myrank,',j=',j
        sumin=0.d0
        sumrf=0.d0
        sumro=0.d0
        roflg=0
        ...
        do i=1,imx1
		   ...
		   !write(*,*) 'myrank=',myrank,',ro(id)=',ro(id)
        end do
         !write(*,*) 'part2,myrank=',myrank
	   ...
	   if(capt(j)<=t) then
              ...
              !if(myrank.eq.3) then ! we don't provide parallel version of writing routines
              ...
              if (outp(6)) then
               ...
               !  if(myrank.eq.2) then ! we don't provide parallel version of writing routines
         ...
         !write(*,*) 'part5,myrank=',myrank
        end do
        end if
     call MPI_barrier(MPI_COMM_WORLD,ierr)
	 write(*,*) 'ppp1,myrank=',myrank    !1 1,3,5  2 2,4
     do j=1,isize
     do k=j,nper,isize
	 !do kk=1,imax
	 Call MPI_BCAST(rik(1+(k-1)*imax),imax,MPI_FLOAT,j-1,MPI_COMM_WORLD,ierr)
     !Call MPI_AllReduce(rik_0,rik,imax*nper,MPI_FLOAT,MPI_SUM,MPI_COMM_WORLD,ierr)
	 !end do
     end do
     end do
     write(*,*) 'part6,myrank=',myrank
     !end do
  else
...
!        print *,"tre",j
  ...
  if(myrank.eq.0) then
      write(*,*) 'irdswm time is: ',time_ed1-time_st1
      write(*,*) 'rnoff compute time is: ',time_ed2-time_st2
  end if 
  return
400 continue
  if(myrank.eq.0) then
