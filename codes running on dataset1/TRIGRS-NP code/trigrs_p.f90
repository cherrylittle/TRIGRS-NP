...
!  allocate (fsmin_0(imax*nout),pmin_0(imax*nout),zfmin_0(imax*nout))
  ...
  if(flag < 0) then
     ...
     if(myrank.lt.nout) then
         do j=myrank+1,nout,isize
        call prpijz(j,u(2),u(19),profil,col,row,header,vrsn) !Revised 12/23/2010
        end do
     endif
  end if
  ...
  if(unsat0) then
     ...
     if(tx==1 .and. nout==1) then
        ...
        if(mmax.gt.0) then	
           ...
           time_end9=MPI_Wtime()
           write(*,*)  myrank, 'call satfin_p time is:',time_end9 - time_begin9
        else
           if(myrank.eq.0) then
     ...
     if (flag<=-4 .or. outp(1)) then ! Moved from computational subroutines 17Nov2014, RLB
        ...
        if (flag<=-7 .and. flag>=-9) then   ! Added 2/10/2012
           ...
           do i=1,imx1
              !newdep=newdep3d(i+(j-1)*imax)
              !dh=dh3d(i+(j-1)*imax)
              newdep=newdep3d_0(i+(j-1)*imax)
              dh=dh3d_0(i+(j-1)*imax)
              !time17_st=MPI_WTIME()
              do k=1,nzs+1
              !  p(k)=p3d(i+(j-1)*imax,k)
               ! thz(k)=th3d(i+(j-1)*imax,k)
                p(k)=p3d_0(i+(j-1)*imax,k)
                thz(k)=th3d_0(i+(j-1)*imax,k)
              end do
              !time17_ed=MPI_WTIME()
              !time16_st=MPI_WTIME()
              call svxmdvp(i,j,dh,newdep,u(19))
