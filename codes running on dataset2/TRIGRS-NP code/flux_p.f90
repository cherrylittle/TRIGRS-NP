...
subroutine flux_p(ic,iper,t0,j1,lcv,ncc,nv0,lwt)
  ...
  flux_time_loop: do  i=1,iper
     ...
     if(tdif1 > 0.0) then
        ...
        if(tstar < smt) then
           ...
           nmn=1
          if(myrank.eq.0.and.threadrank.eq.0) write(*,*) 'cell, time, t* ', ic,t0,tstar, &
          ...
          !if(myrank.eq.0.and.threadrank.eq.0) write(*,*) 'cell, time, t* ', ic,t0,tstar, &
           ! &' Using LTS for basal flux'
        end if
     else 
        qta=0.0
     end if
     !if(myrank.eq.0.and.threadrank.eq.0) write(*,*) 'cell:',ic,'qta:',qta
     tdif2=t0-capt(i+1)
     if(tdif2 > 0.0) then
        tstar=tf*tdif2
        if(tstar < smt) then
     ...
     !if(myrank.eq.0.and.threadrank.eq.0) write(*,*) 'cell:',ic,'qtb:',qtb
     qt=qt+qta-qtb
  ...
  if(qmax<qt .or. qt<0) then ! moved after end of loop, 7 Jan 2013
     ...
     if(dseep < dusz) then
        qt=0.d0
  !   else
   !     if(myrank.eq.0.and.threadrank.eq.0) then
    !       write(*,*) 'Error computing basal flux!','qt:',qt ,'qmax:',qmax!Revised 12/28/2010, RLB
