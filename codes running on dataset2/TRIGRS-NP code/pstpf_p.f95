...
!
subroutine pstpf_p(u1,dhwt,dwt,ulog,i,iper,rf,nccs,lcvs,nmx,t0,jf)
...
select case (flowdir) ! set value of beta (Iverson's beta line)
           ...
           if(tdif2 > 0.0) then
              ...
              if(tstar>5.) then ! solution for later time...
                 series_b_lt: do n=1,mmax
                    ...
                    n1=n
                    if(n>10.and.myrank.eq.0) write (*,*) 'b_lt: n,term2,flt1,tstar',n,term2,flt2,tstar
                    if(delt2<=tol) exit
  ...
  Z_FS_loop: do j=1,nzs+1	    
     ...
     !     if (flag==-3.and.myrank.eq.0) write(u1,'(6(g12.5,1x):)') z,p(j),fs,thz(j) ! added 4/14/2010 RLB,
     !                                                         ! substituted thz for chi 8/22/13, RLB
     ...
     if (jf>0) then
        if(flag<0 .or. outp(1)) then
		   ...
		   p3d(i+(jf-1)*isct(myrank)-idsp(myrank),j)=p(j)
           !newdep3d(i+(jf-1)*imax)=newdep
           !dh3d(i+(jf-1)*imax)=dh
		   newdep3d(i+(jf-1)*isct(myrank)-idsp(myrank))=newdep
		   dh3d(i+(jf-1)*isct(myrank)-idsp(myrank))=dh
        end if
        ...
        if(flag<=-4 .or. outp(1)) th3d(i+(jf-1)*isct(myrank)-idsp(myrank),j)=thz(j)
     end if
     if (fs<fmn) then
        fmn=fs           
        if (jf>0) then
