...
!  Rex L. Baum, 21 Apr 2010, latest revision 07 Feb 2012
	subroutine prpijz(jf,u1,ulog,profil,ncol,nrow,header,vrsn)
	use grids; use input_vars 
	use model_vars
	use input_file_defs
    !use mpi
	implicit none
	integer::i,j,jj,ctr,ncol,nrow,ulog,u1,ctr1,swt,m,nvar,outrow,zval,jf
...
! xmin, ymin at lower left corner of lower left cell, xmax, ymax at upper right corner of upper right grid cell
	  ...
	  !do j=1,nout
	    ...
	    uijz(jf)=jf-1+uijz(1)
        write(*,*) 'uijz(j)',uijz(jf),'jf',jf
	    write(stp,'(i4)') jf
	    stp=adjustl(stp)
	    outfil=trim(folder)//trim(profil)//trim(suffix)//'_'//trim(stp)//'.okc'
	    outfil=adjustl(outfil)
	    open(uijz(jf),file=trim(outfil),status='unknown',err=10)
	    if(deepz>0) then
	      outrow=imax*(nzs+3)
	    else
	      outrow=imax*(nzs+2)
	    end if
	    write(uijz(jf),*,err=11) nvar, outrow, 12 ! last entry on this line not used in all XMDV implemetations, but needed for VisIt to read database correctly RLB 2/21/2012
	    write(uijz(jf),*,err=11) 'x'
	    write(uijz(jf),*,err=11) 'y'
	    write(uijz(jf),*,err=11) 'z'
	    write(uijz(jf),*,err=11) 'p'
	    write(uijz(jf),*,err=11) 'th'
	    write(uijz(jf),fmt='(2(g18.9,1x),i3)',err=11) xmin,xmax,10
	    write(uijz(jf),fmt='(2(g18.9,1x),i3)',err=11) ymin,ymax,10
	    write(uijz(jf),fmt='(2(g15.5,1x),i3)',err=11) zlo,zhi,10
	    write(uijz(jf),fmt='(2(g11.4,1x),i3)',err=11) plo,pmax,10
	    write(uijz(jf),fmt='(2(g11.4,1x),i3)',err=11) th_min,th_max,10
	  !end do
	end if
!
	return
