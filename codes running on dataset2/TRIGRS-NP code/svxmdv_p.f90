...
subroutine svxmdvp(i,jf,delh,newdep,ulog)
  ...
  logical ::ldeep, lopmx,lopmn,loxlo,lz
  integer myrank,isize,ierr,counts
  ...
  if(flag==-7 .or. flag==-8) then
     ...
     z=ztop
	 lz=.false.
	 if(zwat(1)>(zbot*1.00001)) lz=.true.
     if(dcf>0. .and. unsat(zo(i))) then !Unsaturated infiltration model
        do m=1,nzs+1,dinc
           ...
           if(zwat(1) < z .and. zwat(1) > (z-fdinc*zinc) .and. p(m)<0. .and. lz) then ! eliminate 
              !                                       ! repeated entries for water table
              if(zwat(1) > (zbot*1.00001)) then
        ...
        if(ldeep) then
            write(uijz(jf),fmt='(2(g18.9,1x),g15.5,1x,2(g11.4,1x))',err=11) x,y,zdeep,pdeep,th_deep
