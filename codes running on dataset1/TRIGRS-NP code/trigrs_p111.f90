...
program trigrs_mpi
  ...
  DOUBLE PRECISION time_begin3, time_end3
  DOUBLE PRECISION time_begin4, time_end4
  DOUBLE PRECISION time_begin5, time_end5
  DOUBLE PRECISION time_begin6, time_end6
  DOUBLE PRECISION time_begin7, time_end7
  DOUBLE PRECISION time_begin8, time_end8
  DOUBLE PRECISION time_begin9, time_end9
  DOUBLE PRECISION time_begin10, time_end10
  DOUBLE PRECISION time_ini_st, time_ini_ed
  DOUBLE PRECISION time_sav_st, time_sav_ed
  DOUBLE PRECISION time11_st, time11_ed
  DOUBLE PRECISION time12_st, time12_ed
  DOUBLE PRECISION time13_st, time13_ed
  DOUBLE PRECISION time14_st, time14_ed
  DOUBLE PRECISION time15_st, time15_ed
  DOUBLE PRECISION time16_st, time16_ed
  DOUBLE PRECISION time17_st, time17_ed
  DOUBLE PRECISION time_18_st, time_18_ed
  ...
  !call MPI_Wtime(time_begin1)
  time_begin1=MPI_Wtime()
  call trini_p(u(19),u(7),dg2rad)
  !call MPI_Wtime(time_end1)
  time_end1=MPI_Wtime()
  ...
  !call MPI_Wtime(time_begin2)
  time_begin2=MPI_Wtime()
  ...
  allocate (ri(imax),rik(imax*nper),ro(imax),rik_0(imax*nper))
...
!  allocate (fsmin_0(imax*nout),pmin_0(imax*nout),zfmin_0(imax*nout))
  ...
  if(myrank.lt.nout) allocate(fsmin_0(imax*nout))
  !if(myrank.eq.0) allocate(fsmin_0(imax))
  !if(myrank.eq.1) allocate(fsmin_1(imax))
  !if(myrank.eq.2) allocate(fsmin_2(imax))
  allocate (pmin(isct(myrank)*nout))
  if(myrank.lt.nout) allocate(pmin_0(imax*nout))
  !if(myrank.eq.0) allocate(pmin_0(imax))
  !if(myrank.eq.1) allocate(pmin_1(imax))
  !if(myrank.eq.2) allocate(pmin_2(imax))
  allocate (zfmin(isct(myrank)*nout))
  if(myrank.lt.nout) allocate(zfmin_0(imax*nout))
  !if(myrank.eq.0) allocate(zfmin_0(imax))
  !if(myrank.eq.1) allocate(zfmin_1(imax))
  !if(myrank.eq.2) allocate(zfmin_2(imax))
  ...
  time_ini_st=MPI_WTIME()
  if(flag<0 .or. outp(1)) then  !Added 17 Nov 2014, RLB
     !allocate(p3d(imax*nout,nzs+1),p3d_0(imax*nout,nzs+1))
	 allocate(p3d(isct(myrank)*nout,nzs+1))
	 if(myrank.lt.nout) allocate(p3d_0(imax*nout,nzs+1))
     !allocate(dh3d(imax*nout),newdep3d(imax*nout))  
	 ! moved 14 Jan 2015, RLB
	 allocate(dh3d(isct(myrank)*nout))
	 allocate(newdep3d(isct(myrank)*nout))
	 if(myrank.lt.nout) allocate(dh3d_0(imax*nout))
	 if(myrank.lt.nout) allocate(newdep3d_0(imax*nout))
     !allocate(dh3d_0(imax*nout),newdep3d_0(imax*nout))
     dh3d=0.d0;newdep3d=0.d0
	 if(myrank.lt.nout) then
     dh3d_0=0.d0
	 newdep3d_0=0.d0
	 p3d_0=0.d0
	 end if 
  ...
  if (flag<= -4 .or. outp(1)) then ! moved 5/3/2010, flag =-4,...,-9 produces ijz or xyz output 12/22/2010 RLB
     ...
     ix=0;jy=0
     if(flag/=-3) then
	 allocate(th3d(isct(myrank)*nout,nzs+1))
	 if(myrank.lt.nout) then
	 allocate(th3d_0(imax*nout,nzs+1))
	 th3d_0=0.d0
	 end if
	 end if
     th3d=0.d0; 
  ...
  if(flag < 0) then
     ...
     if(myrank.lt.nout) then
        call prpijz(myrank+1,u(2),u(19),profil,col,row,header,vrsn) !Revised 12/23/2010
     endif
  end if
  
  time_ini_ed=MPI_WTIME()
  ! ---------------------------------------------------------------
  if(myrank.eq.0) then
     call date_and_time(date,time)
     write (u(19),*) 'Starting TRIGRS ', vrsn,' ',bldate
  ...
  if(unsat0) then
     ...
     if(mmax.lt.0) then ! infinite depth model
        ...
        !call MPI_Wtime(time_begin5)
        time_begin5=MPI_Wtime()
        call unsinf_p(imx1,u(19),u(2),ncc,nccs)
        !call MPI_Wtime(time_end5)
        time_end5=MPI_Wtime()
        ...
        !call MPI_Wtime(time_begin6)
        time_begin6=MPI_Wtime()
        call unsfin_p(imx1,u(19),u(2),ncc,nccs)
        !call MPI_Wtime(time_end6)
        time_end6=MPI_Wtime()
        write(*,*)  myrank, 'call unsfin_p time is:',time_end6 - time_begin6
     end if
     ...
     if(tx==1 .and. nout==1) then
        ...
        if(mmax.lt.0) then ! infinite depth model
           ...
           !call MPI_Wtime(time_begin7)
           time_begin7=MPI_Wtime()
           call iverson(imx1,u(2),outfil,u(19))
           !call MPI_Wtime(time_end7)
           time_end7=MPI_Wtime()
        else ! finite depth model
           if(myrank.eq.0) then
           ...
           !call MPI_Wtime(time_begin8)
           time_begin8=MPI_Wtime()
           call savage(imx1,u(2),outfil,u(19),nccs)
           !call MPI_Wtime(time_end8)
           time_end8=MPI_Wtime()
        end if
     else
        if(mmax.gt.0) then	
           ...
           nmn=1+mmax ! initialization of nmn added 1/3/2012 RLB
           !call MPI_Wtime(time_begin9)
           time_begin9=MPI_Wtime()
           call satfin_p(imx1,u(19),u(2),nccs)
           time_end9=MPI_Wtime()
           !call MPI_Wtime(time_end9)
        else
           if(myrank.eq.0) then
           ...
           mmax=20; nmn=1+mmax ! initialization of nmn added 1/3/2012 RLB
           !call MPI_Wtime(time_begin10)
           time_begin10=MPI_Wtime()
           call satinf_p(imx1,u(19),u(2),nccs)
           time_end10=MPI_Wtime()
           !call MPI_Wtime(time_end10)
        end if
     end if
  end if
  ! *****************************************************************
...
101 continue
     ...
     if (flag<=-4 .or. outp(1)) then ! Moved from computational subroutines 17Nov2014, RLB
        ...
        if (flag<=-7 .and. flag>=-9) then   ! Added 2/10/2012
           ...
           do i=1,imx1
              !newdep=newdep3d(i+(j-1)*imax)
              !dh=dh3d(i+(j-1)*imax)
              newdep=newdep3d_0(i+(myrank)*imax)
              dh=dh3d_0(i+(myrank)*imax)
              !time17_st=MPI_WTIME()
              do k=1,nzs+1
              !  p(k)=p3d(i+(j-1)*imax,k)
               ! thz(k)=th3d(i+(j-1)*imax,k)
                p(k)=p3d_0(i+(myrank)*imax,k)
                thz(k)=th3d_0(i+(myrank)*imax,k)
              end do
              !time17_ed=MPI_WTIME()
              !time16_st=MPI_WTIME()
              call svxmdvp(i,myrank+1,dh,newdep,u(19))
              !call svxmdv(i,j,dh,newdep,u(19))
              !time16_ed=MPI_WTIME()
           end do
            !time16_st=MPI_WTIME()
            !call svxmdv(j,dh,newdep,u(19))
            !time16_ed=MPI_WTIME()
  ...
  write(*,*) 'call trini_p time is:',time_end1 - time_begin1
  write(*,*) 'call MPI_BCAST time is:',time_end2 - time_begin2
  write(*,*) 'call srdgrd_p time is:',time_end3 - time_begin3
  write(*,*) 'call irdgrd_p time is:',time_18_ed - time_18_st
  write(*,*) 'call rnoff_p time is:',time_end4 - time_begin4
  write(*,*) 'call unsinf_p time is:',time_end5 - time_begin5
  !write(*,*) 'call unsfin_p time is:',time_end6 - time_begin6
  write(*,*) 'call iverson time is:',time_end7 - time_begin7
  write(*,*) 'call savage time is:',time_end8 - time_begin8
  write(*,*) 'call satfin_p time is:',time_end9 - time_begin9
  write(*,*) 'call satinf_p time is:',time_end10 - time_begin10
  write(*,*) 'call ini time is:',time_ini_ed - time_ini_st
  write(*,*) 'call save time is:',time_sav_ed - time_sav_st
  write(*,*) 'call ssvgrd time is:',time11_ed - time11_st
  write(*,*) 'call svijz time is:',time12_ed - time12_st
  write(*,*) 'call flag(-7-9) time is:',time13_ed - time13_st
  write(*,*) 'call svlist time is:',time14_ed - time14_st
  write(*,*) 'call ivgrd time is:',time15_ed - time15_st
  write(*,*) 'call svxmdv time is:',time16_ed - time16_st
  write(*,*) 'call donzs time is:',time17_ed - time17_st
